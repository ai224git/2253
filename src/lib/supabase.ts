...
export async function getFormations(
  ...
  if (filters.type && filters.type.length > 0) {
    const typeConditions = filters.type.map((type: string) => {
      if (type === 'Autres') {
        return `and(not.voie.ilike.%BTS%,not.voie.ilike.%BUT%,not.voie.ilike.%CPGE%,not.voie.ilike.%Licence%,not.voie.ilike.%PASS%)`;
      }
      return `voie.ilike.%${type}%`;
    });
